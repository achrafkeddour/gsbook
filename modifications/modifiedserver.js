...
io.on('connection', (socket) => {
  ...
  socket.on('private-message', ({ to, message }) => {
    ...
    if (recipientSocketId) {
        ...
        io.to(recipientSocketId).emit('private-message', { from: sender, to, message });
        // Also send the message to the sender for display in their chat
        socket.emit('private-message', { from: sender, to, message });
        console.log(`Private message from ${sender} to ${to}: ${message}`);
    } else {
        // Notify the sender that the recipient was not found
        socket.emit('private-message-failed', { to, message: "User not found" });
   ...
   // Broadcast messages to all users
    socket.on('send-broadcast-message', (message) => {
        const sender = users[socket.id];
        io.emit('broadcast-message', { from: sender, message });
    });
